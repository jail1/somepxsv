...
$.summernote.addPlugin({
    name: 'trash', // name of plugin
    buttons: { // buttons
        trash: function () {
            return $.summernote.renderer.getTemplate().iconButton('glyphicon glyphicon-trash', {
                event: 'trash',
                title: 'Shterge'
            });
        }
    ...
    events: { // events
        trash: function (layoutInfo) {
            // Get current editable node
            var $editable = layoutInfo.editable();
            $editable.data('NoteHistory').recordUndo();
            //$log.log($editable.data('NoteHistory'));
            $editable.html('');
            // Call insertText with 'hello'
            //editor.insertText($editable, 'trash');
        }
    }
});
//-----------------
...
angular.module('summernote', [])
    ...
    .controller('SummernoteController', ['$scope', '$attrs', function ($scope, $attrs) {
        ...
        //Bogdan
        summernoteConfig.onToolbarClick = function (evt) {
            $scope.onToolbarClick({evt: evt});
        };
        //^Bogdan
