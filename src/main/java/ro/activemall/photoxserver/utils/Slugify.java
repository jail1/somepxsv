package ro.activemall.photoxserver.utils;
 ...
 * @author Badu
 *
 *         Utility class for slugifying file names
 * 
 *         >>> slugify.slugify(u'Bäuma means a tree', only_ascii=True)
 *         u'bauma-means-a-tree'
 * 
 *         >>> slugify(u'Bakıcı geldi', only_ascii=True) u'bakici-geldi'
 */
public class Slugify {
	...
	public String slugify(String input) {
		...
		if (customReplacements != null) {
			for (Map.Entry<String, String> entry : customReplacements
					.entrySet()) {
		...
		input = Normalizer.normalize(input, Normalizer.Form.NFD)
				.replaceAll("[^\\p{ASCII}]", "").replaceAll("[^\\w+]", "-")
